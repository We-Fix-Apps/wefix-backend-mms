 ...
 * Proxies to backend-shms for centralized file storage
 */
export const uploadFile = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    // Return the response from backend-shms
    res.status(201).json({
      success: true,
      ...
      data: {
        ...
        referenceId: uploadResult.entityId || referenceId,
        referenceType,
        uploadedAt: uploadResult.uploadedAt || uploadResult.createdAt,
      },
    });
  } catch (error: any) {
    console.error('[UploadFile] Error proxying to backend-shms:', error);
    // If proxying fails, fall back to local storage for backward compatibility
    console.warn('[UploadFile] Falling back to local storage');
    ...
    const fileRecord = await File.create({
      ...
      size: file.size,
      category: sanitizeCategory(fileType === FileType.IMAGE ? 'image' : null),
      entityType: referenceType === FileReferenceType.COMPANY ? 'company' : 
                  referenceType === FileReferenceType.CONTRACT ? 'contract' : 
                  referenceType === FileReferenceType.TICKET_ATTACHMENT ? 'ticket' : 'user',
      entityId: referenceId,
      createdBy: user.id,
    } as any) as any;
...
export const uploadMultipleFiles = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  // Upload each file to backend-shms
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      // Proxy upload to backend-shms
      const uploadResult = await fileProxyService.proxyUpload(file, {
        ...req.body,
        ...metadata,
        entityType,
        entityId,
      ...
      uploadedFiles.push({
        ...
        fileSizeMB: uploadResult.fileSizeMB,
        filePath: uploadResult.path || uploadResult.filePath,
        referenceId: uploadResult.entityId || entityId,
        referenceType,
        uploadedAt: uploadResult.uploadedAt || uploadResult.createdAt,
      });
    } catch (error: any) {
      console.error(`[UploadMultipleFiles] Error uploading file ${i} to backend-shms:`, error);
      // If proxying fails for one file, continue with others but log the error
      // Fallback to local storage for this file
      console.warn(`[UploadMultipleFiles] Falling back to local storage for file ${i}`);
